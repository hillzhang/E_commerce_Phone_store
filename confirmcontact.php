...
if(($_POST['captcha']) == $_SESSION['captcha']) {
	...
	$query = "select max(CONTACTID) from contact";
$maxid = OCIParse($db,$query);
if(!$maxid)  {
...
OCIExecute($maxid);
				while(OCIFetch($maxid)) {
					$newcontactid = OCIResult($maxid,1);
				}
				
				$newcontactid++;

 $query = "Insert into contact(CONTACTID, NAME, EMIAL, PHONE, COMPANY, CONTACT) ";
				$query .= " values ('". $newcontactid ."', '". $name ."','". $email ."', '". $phone ."', '". $company ."','". $msg ."')"; 
