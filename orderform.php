...
foreach ($contents as $id=>$qty) {
	...
	/*********************get total and quantity & add in orders, users, orderSummary tables***************************************/		
			...
			/**********************add phones and quantity into orderSummary***********************/
			foreach ($contents as $id=>$qty) {
			$PHONEID = $id;
			$qty = $qty;
		
			$addOrderSummaryQuery = "INSERT INTO orderSummary VALUES ($orderID, $id, $qty)";
			...
			/*---------------------order output-------------------*/
				...
				foreach ($contents as $id=>$qty) {
					...
					$phonesQuery = 'SELECT * FROM phones WHERE PHONEID = '.$PHONEID;
					$phonesQueryParse = OCIParse($db, $phonesQuery); 
  
					if(!$phonesQueryParse)  {
						echo "An error occurred in parsing the sql string.\n"; 
						exit; 
					  }
					OCIExecute($phonesQueryParse);

					while (OCIFetch($phonesQueryParse)) {
						$PHONEID = OCIResult($phonesQueryParse,"PHONEID");
						$company = OCIResult($phonesQueryParse,"COMPANY");
						
						$title = OCIResult($phonesQueryParse,"TITLE");
						
						$year = OCIResult($phonesQueryParse,"YEAR");
						$price = OCIResult($phonesQueryParse,"PRICE");
						$image = OCIResult($phonesQueryParse,"IMAGE");
...
$mysite = 'www.phonestore.com';
$admin = 'Phone Store admin';
$myemail = 'linshan@deakin.edu.au';

$subject = "Phone Store Order Id:  $orderID...";
...
$message = "Customer: $username has just purchased at our store.  
	...
	please check it and dispatch it as soon as possible!!
	
	
	";
	//We can add hyperlink to re-direct to order status of admin page
    
mail('linshan@deakin.edu.au', $subject, $message, "From: $mysite <$myemail>\nX-Mailer:PHP/" . phpversion());
?>
